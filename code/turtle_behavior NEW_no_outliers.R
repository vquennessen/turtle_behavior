...
# import GPS data
# GPS <- read.csv('~/Dropbox/Armando PhD/Chapter 3/Second_Run/Third_run/Dive_variables_NEW/NEW_6_HOUR-SSM/Book3.csv', 
#                 header = TRUE)

GPS <- read.csv('~/Projects/turtle_behavior/data/SUBSET_to_extract_dive_variables.csv',
                header = TRUE)
...
for (t in 1:length(turtle_IDs)) {
  ...
  #remove outliers from surface
  cutoff <- quantile(surfaces$Time_sec, 0.99)
  surfaces <- surfaces %>%
    filter(Time_sec < cutoff)
